...
#1. adding Gaussian corruption to model
    def get_corrupted_input_gaussian(self, input, std_dev, mrg=None):


        if mrg is None:
           mrg = theano_random
           log.debug('Adding Gaussian noise with std: %s', str(std_dev))
           noise = mrg.normal(avg=0, std=std_dev, size=input.shape, dtype=theano.config.floatX)
           OUT = input + noise
        return OUT
    ...
    def get_cost_updates(self, corruption_level, learning_rate):
       ...
       # modified 13/06
        tilde_x = self.get_corrupted_input_gaussian(self.x, corruption_level,mrg=None)
